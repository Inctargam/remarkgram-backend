...
flowchart LR
    ...
    backend["Backend Remarkgram<br/><small>Программная система</small><br/>Авторизует запрос, выполняет soft delete поста<br/>и асинхронно удаляет пост и медиафайлы"]
...
- Remarkgram подтверждает выполнение HTTP-запроса независимо от последующего физического удаления
  медиафайлов.
- Soft-deleted посты физически удаляются планировщиком каждые 12 часов, не более 500 записей за запуск.
- Для связанных медиафайлов задания физического удаления доступны сразу после soft delete;
  планировщик проверяет их каждые 6 часов.
