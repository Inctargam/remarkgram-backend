...
sequenceDiagram
    ...
    rect rgb(245, 240, 255)
        ...
        Files->>FilesDB: Создать задания, доступные сразу
        Files->>FilesDB: Отметить inbox-событие обработанным
    end

    Note over FilesDB,Storage: Планировщик проверяет задания каждые 6 часов
    ...
    Note over Posts,PostsDB: Каждые 12 часов, до 500 записей за запуск
    Posts->>PostsDB: Физически удалить soft-deleted посты
```
...
- Для файлов нет retention-задержки: задания доступны сразу, а планировщик запускается каждые
  6 часов. Повторный `DeleteObject` считается безопасным.
- Soft-deleted посты физически очищаются каждые 12 часов пакетами до 500 записей.
